    ...
    def _build_graph(self):
        ...
        for i, l in enumerate(self._config["hidden_layers"]):
            x = tf.layers.dense(
                x,
                l,
                activation=self._config["hidden_act_fct"],
                kernel_initializer=self._config["weights_init"],
                bias_initializer=self._config["bias_init"],
                name="hidden_%s" % (i),
            )
        ...
        self.mu = tf.layers.dense(
            x,
            self._config["dim"],
            activation=self._config["output_act_fct_mu"],
            kernel_initializer=self._config["weights_init"],
            bias_initializer=self._config["bias_init"],
            name="mu",
        )
        # Getting output for log of Gaussian
        self.log_std = tf.layers.dense(
            x,
            self._config["dim"],
            activation=self._config["output_act_fct_log_std"],
            kernel_initializer=self._config["weights_init"],
            bias_initializer=self._config["bias_init"],
            name="log_std",
        )
