...
class SoftActorCritic:
    ...
    def __init__(
        ...
        **userconfig,
        ...
        self.buffer = ReplayBuffer(
            obs_dim=self._config["dim_obs"], act_dim=self._config["dim_act"], size=self._config["buffer_size"]
        )
    ...
    def _init_update_target_V(self):
        ...
        self._update_target_V_ops_hard = [
            tf.assign(target, source) for target, source in zip(target_params, source_params)
        ]
    ...
    def _prep_train(self):
        ...
        with tf.variable_scope(self._scope, reuse=tf.AUTO_REUSE):
            ...
            if self.alpha == "auto":
                ...
                self.log_alpha = tf.get_variable(
                    name="log_alpha", dtype=tf.float32, initializer=np.log(inital_alpha).astype(np.float32)
                )
                self.alpha = tf.exp(self.log_alpha)
            else:
        ...
        with tf.variable_scope(self._scope, reuse=True):
            ...
            self._Q1_pi = self._v_fct(
                inp=tf.concat([self.obs, self._Policy.act], axis=-1), scope="Q_func1", **self._q_fct_config
            )
            self._Q2_pi = self._v_fct(
                inp=tf.concat([self.obs, self._Policy.act], axis=-1), scope="Q_func2", **self._q_fct_config
            )
        ...
        self.Q1_loss = 0.5 * tf.reduce_mean(
            (
                tf.stop_gradient(self.rew + self._config["discount"] * (1 - self.done) * self._V_target.output)
                - self._Q1.output
            )
            ** 2
        )
        ...
        self._train_opQ1 = self.Q1optim.minimize(
            loss=self.Q1_loss, var_list=self._vars(self._Q1._scope), name="AdamQ1_min"
        )  #########################
        ...
        with tf.control_dependencies([self._train_opQ1]):
            self._train_opQ2 = self.Q2optim.minimize(
                loss=self.Q2_loss, var_list=self._vars(self._Q2._scope), name="AdamQ2_min"
            )  #########################
        ...
        with tf.control_dependencies([self._train_opQ2]):
            self._train_opV = self.Voptim.minimize(
                loss=self.V_loss, var_list=self._vars(self._V._scope), name="AdamV_min"
            )  #########################

        # PI update
        self.PI_loss_KL = tf.reduce_mean(self.alpha * self.log_prob_new_act - self._Q1_pi.output)
        self.policy_regularization_loss = (
            0.001 * 0.5 * (tf.reduce_mean(self._Policy.log_std**2) + tf.reduce_mean(self._Policy.mu**2))
        )
        ...
        with tf.control_dependencies([self._train_opV]):
            self._train_opPI = self.PIoptim.minimize(
                loss=self.PI_loss, var_list=self._vars(self._Policy._scope), name="AdamPi_min"
            )  #########################

        if self.train_alpha:
            with tf.control_dependencies([self._train_opPI]):
                self.alpha_loss = -tf.reduce_mean(
                    self.log_alpha * tf.stop_gradient(self.log_prob_new_act + self.target_entropy)
                )
                self.alpha_optimizer = tf.train.AdamOptimizer(
                    learning_rate=self._config["lambda_Alpha"], name="AdamAlpha"
                )
                self._train_opAlpha = self.alpha_optimizer.minimize(
                    self.alpha_loss,
                    var_list=tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, scope=self._scope + "/log_alpha"),
                    name="AdamAlpha_min",
                )
    ...
    def _train(self):
        ...
        if self.train_alpha:
            ...
            _, loss_PI_fct, _, loss_V_fct, _, loss_Q1_fct, _, loss_Q2_fct, _, _, _ = self._sess.run(
                train_ops, feed_dict=fddct
            )
        else:
            train_ops = [
                self._train_opPI,
            ...
            _, loss_PI_fct, _, loss_V_fct, _, loss_Q1_fct, _, loss_Q2_fct, _ = self._sess.run(
                train_ops, feed_dict=fddct
            )
